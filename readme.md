# streamlit_pr

## 역할 분담

- 최다온: 음식물 쓰레기 발생량 기록 & 감소 분석, 포인트 제도 (Gamification)
- 유용우: 음식정보 csv파일 만들기, 쓰레기/플라스틱 분리배출 가이드
- 김영우: 소비기한 알리미, 식재료 보관 방법 TIP
- 김재민: 레시피추천, 칼로리 & 영양 정보

# 🧊 냉장고를 지켜줘  
Streamlit과 SQLite 데이터베이스 기반의 냉장고 관리 서비스로, 식재료 소비기한 알림·레시피 추천·음식물 쓰레기 감소 분석 등을 제공하는 스마트 식재료 관리 시스템입니다.

---



## 📘 프로젝트 소개

### 📝 가제  
**냉장고를 지켜줘 (Save My Fridge)**

### 📝 한줄 소개  
냉장고 속 식재료를 입력하면, 소비기한·레시피·쓰레기 배출량·보관법 등을 알려주는 **스마트 냉장고 관리 서비스**

### 🎯 프로젝트 목적  
- 1인 가구의 식재료 낭비 줄이기

- 유통기한 관리 자동화 및 데이터베이스화

- 음식물 쓰레기 줄이기

- 배달 음식 재사용까지 포함한 종합 식생활 관리 서비스 구축

---

## 👥 역할 분담
- 최다온: 음식물 쓰레기 발생량 기록 & 감소 분석, 포인트 제도 (Gamification)

- 유용우: 초기 데이터(CSV) 구축 및 DB 연동, 쓰레기/플라스틱 분리배출 가이드

- 김영우: 소비기한 알리미, 식재료 보관 방법 TIP

- 김재민: 레시피추천, 칼로리 & 영양 정보

## 🔥 Core Features (핵심 기능)

### 1️⃣ 소비기한 알리미 (Expiration Alerts)
- 입력된 식재료의 소비기한·유통기한을 자동 계산  
- 임박한 순서대로 정렬  
- 색상/지표(metric) 기반 시각화  
- Streamlit metric 기능(Task 2 응용)

---

### 2️⃣ 레시피 추천 (+배달 음식 활용)
- 냉장고 속 재료 기반 레시피 추천  
- 부족한 재료 표시  
- 배달 음식 업사이클 레시피 제공  
  - 예: 남은 치킨 → 치킨마요덮밥, 닭죽  
  - 남은 김치/소스 활용법

---

### 3️⃣ 음식물 쓰레기 발생량 기록 & 감소 분석
- 버려진 음식물 기록  
- 주간·월간 기준 음식물 쓰레기 증가/감소량 비교  
- 바 차트/라인 차트로 시각화 (Task 4 chart 사용 가능)  
- 감소량 기반 포인트 보상 기능과 연동

---

### 4️⃣ 쓰레기/플라스틱 분리배출 가이드
- 식재료 껍질·포장지의 분리배출 기준 제공  
- 배달 음식 용기 종류별 올바른 재활용 안내  
- 분리배출 실천 시 포인트 제공 가능

---

### 5️⃣ 식재료 보관 방법 TIP
- 식재료별 적정 보관 온도/습도  
- 냉동/냉장 구분 기준  
- 저장 기간 및 보관 팁 제공  
- Tabs/Expander(Task 6) 활용하여 정돈된 UI 구성 예정

---

### 6️⃣ 칼로리 & 영양 정보
- 레시피 또는 재료 기반 칼로리 계산  
- 1인분 기준 칼로리 표시  
- 향후 탄단지 비율까지 확장 가능  

---

### 7️⃣ 포인트 제도 (Gamification)
포인트 누적을 통해 사용자 참여를 유도하는 게이미피케이션 기능

---

### 8️⃣ 데이터 관리 및 저장소 (Database)
- 본 프로젝트는 SQLite를 도입하여 데이터의 지속성(Persistence)을 보장합니다.

- SQLite DB 연동: 로컬 데이터베이스(fridge.db)를 구축하여 서버 재시작 시에도 데이터가 유지됩니다.

- CSV 자동 로드: 앱 초기 실행 시 food_data.csv (기본 식재료 50여 종의 보관법/분리배출 데이터)를 자동으로 DB에 적재합니다.

- 테이블 구조:
- ingredients: 식재료 명, 수량, 소비기한, 보관팁, 분리배출 정보
- waste_log: 음식물 쓰레기 배출 날짜 및 양(g) 로그
- user_points: 사용자 포인트 적립 내역 및 레벨 관리


**포인트 예시 기준**
- 임박 재료 사용: +30점  
- 음식물 쓰레기 감소: +40점  
- 배달음식 업사이클 레시피 실행: +20점  
- 1주일 동안 쓰레기 발생량 감소: +50점  

**활용 예**  
- 레벨/뱃지 부여  
- 친환경 등급 시스템  
- 진행도 바(Task 7 progress bar 응용)

---

## 🧠 브레인스토밍 (초기 아이디어)
> 보고서 제출 시 아래 이미지 포함  
- 식재료 유통기한 관리  
- 레시피 자동 추천  
- 배달 음식 재활용  
- 쓰레기 처리 가이드  
- 음식물 쓰레기 감소 피드백  
- 포인트 제도 도입  

---

## 🛠 기술 스택 (Tech Stack)

| 분야 | 사용 기술 |
|------|-----------|
| 언어 | Python 3.x |
| 프레임워크 | Streamlit |
| 데이터베이스 | SQLite3 |
| 시각화 | Streamlit chart, bar chart |
| 기타 | PIL, numpy |

---

## 🧪 사용 흐름 (User Flow)

1️⃣ 냉장고에 있는 식재료 입력  
2️⃣ 소비기한 자동 계산 → 임박 재료 알림  
3️⃣ 레시피 추천 (냉장고·배달음식 모두 지원)  
4️⃣ 요리 후 남은 쓰레기 처리 가이드 확인  
5️⃣ 음식물 쓰레기 감소량 분석  
6️⃣ 포인트 적립으로 동기부여  